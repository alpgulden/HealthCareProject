...
# categories = df_mix_temp.select("AGE").distinct().rdd.flatMap(lambda x: x).collect()

# exprs = [when(col("AGE") == category, 1).otherwise(0).alias(category) for category in categories]

# df_mix_temp = df_mix_temp.select('SUBJECT_ID', 'GENDER', 'LOS', 'EXPIRE_FLAG', 'ICD9_LIST', *exprs)
...
featured_data = assembler.transform(df_all_features).select('features', 'EXPIRE_FLAG')
...
classifiers = [LogisticRegression(labelCol='EXPIRE_FLAG'),
               ...
               MultilayerPerceptronClassifier(labelCol='EXPIRE_FLAG', layers=[34, 20, 20, 2]),
